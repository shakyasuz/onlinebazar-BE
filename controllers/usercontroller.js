...
//create aroute for user registration
router.post('/register', async(req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            return res.status(400).json({ msg: "User already exists" }); //return stops 
        }
        //hash the password
        const salt=await bcrypt.genSaltSync(10);
        const passHash=await bcrypt.hashSync(pass,salt);
...
router.post("/login",async (req,res)=>{
    ...
    try{
        ...
        //create a token and signing it with jwt
